...
# for csv saving
# setting_names = ['mu', 'N', 'sigma', 'num_group', 'a', 'b', 'acc_prob', 'beta', 'rec_how', 'acc_how', 'intervention_end', 'node_removal', 'edge_removal', 'is_ab_test', 'treatment_probability', 'treatment_size', 'treatment_time']
...
def run(mu, N, sigma, a, b, acc_prob, beta, rec_how, acc_how, intervention_end=None, node_removal=False, edge_removal=False, is_ab_test=False, treatment_probability=1, treatment_size=None, treatment_time=50):
    ...
    grp_info = [{'mean': mu[i], 'variance': sigma[i]*np.identity(2), 'size':N[i]} for i in range(len(N))]
    ...
    if treatment_size == None:
        treatment_size = 'na'

    # for csv saving
    # settings = [mu, N, sigma, num_group, a, b, acc_prob, beta, rec_how, acc_how, intervention_end, node_removal, edge_removal, is_ab_test, treatment_probability, treatment_size, treatment_time]
    # metrics = []
    ...
    # for json saving
    settings = {'mu':[m.tolist() for m in mu], 'N':N, 'sigma':sigma, 'num_group':num_group, 'a':a, 'b':b, 'acc_prob':acc_prob, 'beta':beta, 'rec_how':rec_how, 'acc_how':acc_how, 'intervention_end':intervention_end, 'node_removal':node_removal, 'edge_removal':edge_removal, 'is_ab_test':is_ab_test, 'treatment_probability':treatment_probability, 'treatment_size':treatment_size, 'treatment_time':treatment_time}
...
settings = list(product(mu, N, sigma, a, b, acc_prob, beta, rec_how, acc_how, intervention_end, node_removal, edge_removal, is_ab_test, treatment_probability))
